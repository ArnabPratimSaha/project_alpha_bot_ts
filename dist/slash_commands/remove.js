...
const remove = (interaction) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c, _d;
    try {
        ...
        if (!interaction.member.roles.cache.find(r => r.id === guildData.roleID) || !interaction.member.permissions.has('ADMINISTRATOR')) {
            yield interaction.reply({
                content: 'You dont have access to update a user',
                ephemeral: true
            });
            return;
        }
        ...
        if (!guildData.validMembers.find(v => v === data.value)) {
            yield interaction.reply({
                content: `member <@${data.value}> not found in the list`,
                ephemeral: true
            });
            return;
        }
            ...
            guildData.validMembers = guildData.validMembers.filter(m => m !== data.value);
            ...
            try {
                const roleMember = yield guildMember.roles.remove(role);
                if (roleMember.roles.cache.find(r => r.id === guildData.roleID)) {
                    yield guildData.save();
                    yield interaction.reply({
                        content: `member <@${data.value}> successfully removed from the list`,
                        ephemeral: true
                    });
                    return;
                }
                ...
                yield guildData.save();
                yield interaction.reply({
                    content: `member <@${data.value}> successfully removed from the list.\nBut ${viviRole === null || viviRole === void 0 ? void 0 : viviRole.name} can not be removed`,
                    ephemeral: true
                });
                return;
            }
