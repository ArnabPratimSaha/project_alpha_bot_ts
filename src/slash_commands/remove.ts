...
const remove=async(interaction:CommandInteraction):Promise<void>=>{
    try {
        ...
        if(!guildData.validMembers.find(v=>v===data.value)){
            ...
            try {
                const roleMember=await guildMember.roles.remove(role);
                ...
                await guildData.save();
                await interaction.reply({
                    content:`member <@${data.value}> successfully removed from the list.\nBut ${viviRole?.name} can not be removed`,
                    ephemeral:true
                })
                return;
            }
